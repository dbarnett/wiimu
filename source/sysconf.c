...
THIS IS A MODIFIED VERSION WHICH INCLUDES SYSCONF_GetShopCode,
AND SYSCONF_SetShopCode! THIS IS NOT THE ORIGINAL VERSION!
...
s32 SYSCONF_GetShopCode(void)
{
	int res;
	u8 country = 0;

	if (SYSCONF_GetLength("IPL.SADR") != SADR_LENGTH) return SYSCONF_EBADVALUE;
	res = SYSCONF_Get("IPL.SADR", __sadr, SADR_LENGTH);
	if(res < 0) return res;
	if(res!=1) return SYSCONF_EBADVALUE;
	country = __sadr[0];
	return country;
...
s32 SYSCONF_SetShopCode(u8 country)
{
	SYSCONF_GetShopCode();
	__sadr[0]=country;
	return SYSCONF_Set("IPL.SADR", __sadr, 1);
}
